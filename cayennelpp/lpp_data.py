...
from cayennelpp.lpp_util import *
...
class LppData(object):
    ...
    def __init__(self, chn, tid, data):
        logging.debug("LppData(channel=%d, type=%d, len=%d)", chn, tid, len(data))
        if not tid in LPP_DATA_TYPE:
    ...
    def __str__(self):
        return 'LppData(channel = {}, type = {}, data = {})'.format(self._chn, LPP_DATA_TYPE[self._tid]['name'], str(self._dat))

    @classmethod
    def from_bytes(class_object, bytes):
        logging.debug("LppData.from_bytes: bytes=%s, length=%d", bytes.hex(), len(bytes))
        if len(bytes) < 3:
    ...
    def data(self):
        ...
        logging.debug("  out:   values = %s, num = %d", str(self._dat), len(self._dat))
        return self._dat
...
def main():
    ...
    print("Temperature: data=%s, bytes=%s, data_size=%d, bytes_size=%d" %
          (temp_dat.data(), temp_dat.bytes().hex(), temp_dat.data_size(), temp_dat.bytes_size()))
    ...
    print("Accelerometer: data=%s, bytes=%s, data_size=%d, bytes_size=%d" %
          (acc_dat.data(), acc_dat.bytes().hex(), acc_dat.data_size(), acc_dat.bytes_size()))
    # 01 88 06 76 5f f2 96 0a 00 03 e8
    gps_buf = bytearray([ 0x01, 0x88, 0x06, 0x76, 0x5f, 0xf2, 0x96, 0x0a, 0x00, 0x03, 0xe8 ])
    gps_dat = LppData.from_bytes(gps_buf)
    print("GPS: data=%s, bytes=%s, data_size=%d, bytes_size=%d" %
          (gps_dat.data(), gps_dat.bytes().hex(), gps_dat.data_size(), gps_dat.bytes_size()))
